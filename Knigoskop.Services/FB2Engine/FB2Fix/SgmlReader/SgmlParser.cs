...
namespace FB2Fix.Sgml
{
    ...
    internal class Entity
    {
        ...
        public char SkipWhitespace(IList<char> skipped)
        {
            char ch = Lastchar;
            while (ch != EOF && (ch == ' ' || ch == '\r' || ch == '\n' || ch == '\t'))
            {
        ...
        public string ScanToken(StringBuilder sb, string term, bool nmtoken)
        {
            ...
            {
                throw new Exception(String.Format("Invalid name start character '{0}'", ch));
            }
            while (ch != EOF && term.IndexOf(ch) < 0)
            {
                ...
                {
                    throw new Exception(String.Format("Invalid name character '{0}'", ch));
                }
                ch = ReadChar();
            }
            return sb.ToString();
            ...
            while (ch != EOF)
            {
                ...
                else if (state > 0)
                {
                    ...
                    if (sb != null)
                    {
                        ...
                        {
                            sb.Append(ch);
                        } // if not then append it to buffer.
                    }
                    state = newstate;
        ...
        static int[] CtrlMap = new int[] { 8364, 129, 8218, 402, 8222, 8230, 8224, 8225, 710, 8240, 352, 8249, 338, 141, 381, 143, 144, 8216, 8217, 8220, 8221, 8226, 8211, 8212, 732, 8482, 353, 8250, 339, 157, 382, 376};
    ...
    internal class HtmlStream : TextReader
    {
        ...
        internal Decoder AutoDetectEncoding(byte[] buffer, ref int index, int length)
        {
            if (4 <= (length - index))
            {
                ...
                switch (w)
                {
                    case 0xfeff:
                        index += 2;
                        this.encoding = System.Text.Encoding.BigEndianUnicode;
    ...
    internal class ElementDecl
    {
        ...
        public ElementDecl(string name, bool sto, bool eto, ContentModel cm, string[] inclusions, string[] exclusions)
        {
            ...
            if(inclusions != null)
            {
                this.inclusions = new Set<string>(inclusions.Length);
                this.inclusions.AddRange(inclusions);
            }

            if (exclusions != null)
            {
                this.exclusions = new Set<string>(exclusions.Length);
        ...
        public bool CanContain(string name, SgmlDtd dtd)
        {
            ...
            {
                if(this.inclusions.Contains(name))
                {
                    return true;
                }
            }
    ...
    internal class Group
    {
        ...
        public void AddConnector(char c)
        {
            ...
            {
                throw new Exception(String.Format("Connector '{0}' is inconsistent with {1} group.", c, this.GroupType));
            }
        ...
        public bool CanContain(string name, SgmlDtd dtd)
        {
            ...
            foreach (object obj in this.members)
            {
                ...
                if (symbol != null)
                {
                    ElementDecl elementDecl = dtd.FindElement(symbol);
                    
                    if (elementDecl != null)
                    {
                        if (elementDecl.StartTagOptional)
                        {
                            // tricky case, the start tag is optional so element may be allowed inside this guy!
                            if (elementDecl.CanContain(name, dtd))
                            {
                                return true;
                            }
                        }
    ...
    internal class AttList : IEnumerable
    {
        Hashtable AttDefs;
    ...
    internal class SgmlDtd
    {
        public string Name;

        Dictionary<string, ElementDecl> elements;
        Hashtable pentities;
        Hashtable entities;
        StringBuilder sb;
        Entity current;
        ...
        public SgmlDtd(string name, XmlNameTable nt)
        {
            this.nameTable = nt;
            this.Name = name;
            this.elements = new Dictionary<string, ElementDecl>(StringComparer.CurrentCultureIgnoreCase);
                ...
                switch (ch)
                {
                    ...
                    case '%':
                        ...
                        catch (Exception ex)
                        {
                            ...
                            ApplicationLogger.WriteStringToError(ex.Message + this.current.Context());
                        }
                        ch = this.current.Lastchar;
                        break;
                    default:
        ...
        void ParseMarkup()
        {
            ...
            if (ch != '!')
            {
                this.current.Error("Found '{0}', but expecing declaration starting with '<!'", ch);
                return;
            }
            ch = this.current.ReadChar();
            if (ch == '-')
            {
                ...
                if (ch != '-')
                {
                    this.current.Error("Expecting comment '<!--' but found {0}", ch);
                }
        ...
        char ParseDeclComment(bool full)
        {
            ...
            if (full && ch != '-')
            {
                this.current.Error("Expecting comment delimiter '--' but found {0}", ch);
            }
        ...
        void ParseIgnoreSection()
        {
            ...
            if (ch != '[')
            {
                this.current.Error("Expecting '[' but found {0}", ch);
            }
        ...
        void ParseEntity()
        {
            ...
            if (pe)
            {
                this.pentities.Add(e.Name, e);
            }
            else
            {
                this.entities.Add(e.Name, e);
            }
        }
